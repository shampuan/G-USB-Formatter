...
# Sabitler
APP_NAME = "G-USB Formatter 3.0.2"
APP_VERSION = "3.0.2" 
...
class USBFormatterApp(QWidget):
    ...
    def load_disks(self):
        ...
        self.disk_sizes = {}
        try:
            # Bağlantı türünü (usb, sata, nvme vb.) anlamak için TRAN sütununu da ekliyoruz
            res = subprocess.run([COMMAND_PATHS['lsblk'], '-o', 'NAME,RM,TYPE,SIZE,MODEL,TRAN', '--json', '-p'], capture_output=True, text=True)
            if res.returncode == 0:
                data = json.loads(res.stdout)
                devices = data.get("blockdevices", [])
                for dev in devices:
                    is_disk = dev.get("type") == "disk"
                    is_removable = str(dev.get("rm")).strip() in ["1", "true", "True"]
                    transport = str(dev.get("tran", "")).lower().strip()
                    
                    # EMNİYET FİLTRESİ: 
                    # Cihaz ya çıkarılabilir olmalı (RM=1) YA DA bağlantı tipi 'usb' olmalı (Harici HDD kutuları için)
                    if is_disk and (is_removable or transport == "usb"):
                        name = dev.get("name", "")
                        size_str = dev.get("size", "0B")
                        model = dev.get("model", "").strip()
                        
                        if name:
                            self.disk_sizes[name] = self.parse_size(size_str)
                            # Eğer model bilgisi boşsa (RAW disklerde olabilir), yanına "External USB Drive" yazalım
                            if not model:
                                model = "External USB Drive"
                            
                            display_text = f"{name} ({size_str}) {model}"
                            self.disk_combo.addItem(display_text)
